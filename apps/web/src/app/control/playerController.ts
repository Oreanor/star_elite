...
export function createPlayerController(intent: PlayerIntent): Controller {
  ...
  return {
    update(ship: ShipEntity, world: World, dt: number): void {
      ...
      // Миелофон: масштаб борта. `=` растит, `−` уменьшает, удержанием. Пока дев-доступ
      // (без гейта модулем): растить может любой — управляющий сигнал, а домен его исполнит.
      // Мир при этом на глаз УМЕНЬШАЕТСЯ (камера отъезжает ∝ масштабу), корабль как был.
      c.grow = (isHeld('Equal') ? 1 : 0) - (isHeld('Minus') ? 1 : 0)
    },
