...
function systemPrompt(ctx: NegotiationContext): string {
  ...
  return [
    ...
    '',
    `Перед тобой командир на «${y.ship}» в ${ctx.distanceM} м. У него корпус ${y.hullPct}%, в трюме: ${y.cargo}. Он ${ctx.yourHeading}.`,
    ...
    '— Трепаться можно о чём угодно: погода, слухи, куда летишь. Опирайся на факты выше, не выдумывай мир.',
