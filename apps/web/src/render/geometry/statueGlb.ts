...
/**
 * Статуи-монолиты из GLB. Три облика; какой где — решает домен (`MonolithEntity.variant`).
 *
 * Как и у станций, геометрию НОРМИРУЕМ в единичную сферу и ЦЕНТРИРУЕМ: тогда внешний
 * `scale = radius` даёт нужный габарит (у статуи это километры), и модель можно менять, не трогая
 * числа домена. Материал — родной, лишь пригашенный: Meshy пекёт свет в emissive, и без этого
 * камень «светится» изнутри.
 */
