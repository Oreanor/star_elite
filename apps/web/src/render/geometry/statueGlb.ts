...
/**
 * Реестр обликов. Порядок = `MonolithEntity.variant` = `MONOLITH_NAMES`. В исходниках
 * (`docs/models/statues/`) лежат и запасные меши — в `public/models` только то, что рисуется:
 * GLB грузятся ЖАДНО при импорте модуля, и лишний файл — его текстуры в видеопамяти навсегда.
