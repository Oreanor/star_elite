...
/**
 * ТЕКСТУРЫ КАЖДОЙ МОДЕЛИ УЖИМАЮТСЯ ДО 1024². Это бюджет видеопамяти, и цена ошибки тут
 * сильно больше, чем кажется по файлу на диске.
 *
 ...
 * несжатой: 2048² × RGBA × мипы = 22 МБ КАЖДАЯ, 89 МБ на модель, 1.16 ГБ на все тринадцать.
 * 1024² даёт 5.6 МБ на карту и 291 МБ на парк — вчетверо меньше, и на глаз не отличить.
 *
 * ПОЧЕМУ НЕ МЕНЬШЕ, хотя памяти это сняло бы куда больше: пробовали 256² (18 МБ на парк) —
 * «жуткое мыло». Модели текстурные, а не крашеные по вершинам: весь рисунок корпуса живёт
 * в карте цвета, и вместе с разрешением уходит он. Рассуждение «корабль занимает пару сотен
 * пикселей, значит хватит четверти тысячи» неверно — корабль разглядывают вблизи, в верфи и
 * на облёте камерой. 1024 — найденный глазом предел, ниже начинается каша.
 *
 * ЭТИМ И ЛЕЧИЛАСЬ ПРОСАДКА 60 → 30. Полигонаж был образцовый (~1000 тришек на корабль),
 * draw call-ов в кадре пять — ломаться было нечему, кроме памяти. Гигабайт текстур на игру,
 * которой хватает трёх сотен мегабайт, — и кадр падал вдвое.
 *
 * ЗАМЕРЯЕШЬ — ПЕРЕЗАГРУЗИ СТРАНИЦУ. На этом сгорел целый заход: модели грузятся ОДИН раз, при
 ...
 *
 * Грузим ВСЕ корпуса жадно, при импорте модуля, — вот почему бюджет считается по ВСЕМУ
 * реестру, а не по тому, что сейчас в кадре.
 */
const GLB_HULLS: readonly GlbHullDef[] = [
  ...
  // «Атлас» — корабль поколений: крупнее прочих (scale выше).
  { id: 'atlas', url: '/models/atlas.glb', scale: 24, yaw: GLB_YAW, pitch: GLB_PITCH, roll: GLB_ROLL },
]
