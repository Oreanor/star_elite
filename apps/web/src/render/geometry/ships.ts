...
/**
 * ТЕКСТУРЫ КАЖДОЙ МОДЕЛИ ОБЯЗАНЫ БЫТЬ УЖАТЫ ДО 256². Это не вкус, это бюджет видеопамяти,
 * и цена ошибки тут больше, чем кажется по файлу на диске.
 *
 ...
 * Ровно так игра и просела с 60 кадров до 30 — при том, что полигонаж образцовый (~1000
 * тришек на корабль) и draw call-ов в кадре пять. Полигоны были ни при чём.
 *
 * 256² даёт 350 КБ на карту и 18 МБ на весь парк — в шестьдесят раз меньше. Разницы не видно:
 * корабль занимает пару сотен пикселей экрана, палитра узкая, цвета плоские.
 *
 * Шаг пайплайна (после `dequantize`, перед укладкой в `public/models`):
 *   npx @gltf-transform/cli resize --width 256 --height 256 in.glb out.glb
 *
