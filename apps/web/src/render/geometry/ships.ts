...
const GLB_HULLS: readonly GlbHullDef[] = [
  // «Святой Дух» (spiritus.glb) — корабль игрока и бога. Масштаб ОБЩИЙ, как у прочих
  // корпусов: экспорт у него обычный, поправка просто делала бы борт вдвое мельче Авроры при
  // той же раме. Радиус шасси (12 м) не менялся, так что сфера попаданий совпадает с бортом.
  //
