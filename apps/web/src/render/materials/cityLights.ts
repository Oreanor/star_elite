...
void main() {
  ...
  // Порог: светится меньшинство клеток. uDensity — доля обитаемых на суше.
  float density = uDensity * belt * land;
  float lit = step(1.0 - density, seed);

  // Внутри клетки огонёк не заполняет её целиком — иначе выйдет шахматная доска.
  float spark = 1.0 - smoothstep(0.2, 0.5, length(fract(grid) - 0.5));
  ...
  glow = mix(density * 0.18, glow, resolved);
 ...
 * а столица в четыре миллиарда — сплошной сеткой. Это данные, а не два шейдера.
 */
