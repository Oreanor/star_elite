...
export function RemotePlayers() {
  ...
  useFrame((_, dt) => {
    ...
    // 2) Кто должен быть в мире: онлайн-игроки в МОЕЙ системе и не «отошедшие»
    //    (пауза = вышел из игры, борт гаснет). Имя/вид/лицо берём из presence.
    const want = new Map<string, (typeof peersRef.current)[number]>()
    for (const p of peersRef.current) {
      if (p.systemIndex === sys && !p.paused) want.set(p.uid, p)
    }
