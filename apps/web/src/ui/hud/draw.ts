...
export function corners(ctx: CanvasRenderingContext2D, x: number, y: number, size: number, color: string) {
  ...
  for (const sx of [-1, 1]) {
    for (const sy of [-1, 1]) {
      const cx = x + sx * half
      const cy = y + sy * half
      line(ctx, cx, cy, cx - sx * arm, cy, color)
      line(ctx, cx, cy, cx, cy - sy * arm, color)
    }
  }
}
