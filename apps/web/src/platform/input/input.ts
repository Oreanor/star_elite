...
export function attachInput(canvas: HTMLCanvasElement): () => void {
  ...
  const onKeyDown = (e: KeyboardEvent) => {
     ...
     * действие браузера у любой клавиши. Иначе сочетания с Ctrl уходят браузеру:
     * Ctrl+R перезагрузит страницу, Ctrl+L уведёт фокус в адресную строку, Ctrl+P
     * откроет печать, Ctrl+S — сохранение. Каждое рвёт захват прямо посреди боя, а
     * торможение висит на Ctrl — значит перехват случался бы на каждом манёвре с
     * зажатым тормозом. Заодно эта же клавиша, отпущенная уже за пределами игры,
     * не доносит до нас keyup и «залипает»; не пуская сочетание к браузеру, мы
     * держим захват, а с ним и штатный keyup.
     *
     * Escape выше намеренно не глушим: это возврат мыши, он обязан работать всегда.
     * Вне захвата (меню, пауза) браузер снова свой — там гасим лишь прокрутку от
     * пробела, стрелок и Tab.
     */
