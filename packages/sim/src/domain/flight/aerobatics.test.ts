...
import { forward, shipAxes } from './axes'
...
describe('фигуры пилотажа', () => {
  ...
  it('бочка сохраняет и курс, и линию полёта', () => {
    ...
    // Бочка — РОВНО один оборот (около 2 с), а не «крутись до предохранителя».
  ...
  it('бочка выходит РОВНЫМ: тормозит во второй половине, а не клинит в крене', () => {
    const { world } = fly('barrel')
    const right = new Vector3()
    shipAxes(world.player.state.quat, new Vector3(), right, new Vector3())
    const bankDeg = Math.abs((Math.atan2(right.y, right.x) * 180) / Math.PI)
    // Встречный крен во второй половине оборота гасит вращение к нулю у ровного —
    // корабль не замирает завалившись набок и не встаёт колом на полном ходу.
    expect(bankDeg).toBeLessThan(6)
  })
