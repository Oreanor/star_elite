...
export function stepGravity(ship: ShipEntity, world: World, dt: number): void {
  ...
  /**
   * НА ПОСАДКЕ гравитация молчит. Ховер держит корабль над самой поверхностью, и притяжение
   * там только мешает: оно копит вниз скорость, которую режим тут же гасит, а на взлёте
   * эта копилка отдаётся рывком вниз. Пилот в ховере занят не борьбой с тягой.
   *
   * В ПОЛЁТЕ она работает как работала. Автоматически держать высоту — нечестно: хочешь
   * ниже — ныряй носом сам, и плати за это высотой.
   */
