 ...
 */
/**
 * Радиус причала, м. Один на все станции: облик выбирается сидом отдельно, а габарит
 * общий. Стыковка и защитное поле считаются долями ОТ НЕГО (`DOCKING.RANGE_FACTOR`,
 * `SHIELD.RADIUS_FACTOR`), поэтому число можно менять, не трогая ничего больше.
 */
export const STATION_RADIUS = 1_000
...
export const STARTER_SYSTEM: SystemDef = {
   ...
   */
  station: { name: 'Кориолис «Октанеар»', pos: [STATION_ORBIT, 0, -AU], radius: STATION_RADIUS },
  /**
   * Дверь во вселенную: чёрный шарик у причала. Влетевший в него уходит на куст галактик
   * (см. docs/hyperbolic_bush_plan.md).
   *
   * Смещение — шестьдесят километров, а не «пара тысяч рядом с кольцом». Игрок стартует
   * в двух километрах от причала, и дыра под боком возмущала бы гравитацией всё, что летает
   * в стартовой зоне: ракеты уводило с курса, борта тормозило. Отсюда её видно как чёрную
   * точку у станции, но ни в стыковку, ни в бой она не вмешивается.
   */
  blackHoles: [
    { name: 'Дверь', radius: 300, stationOffset: [0, 0, 60_000] },
  ],
