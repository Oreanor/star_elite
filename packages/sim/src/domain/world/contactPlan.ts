...
export type RawPlanStep =
  ...
  /** Подлететь к текущей цели навигации игрока (станция/планета — тело едет, ведём по id). */
  | { step: 'approach-nav' }
  /** Подлететь К САМОМУ ИГРОКУ и встать рядом («подлети ко мне»). Не путать с `approach-nav`. */
  | { step: 'come' }
