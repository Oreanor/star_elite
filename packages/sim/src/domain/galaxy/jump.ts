 ...
 *
 * ВРЕМЕННОЕ. Настоящая вселенная — куст из `domain/universe`: тысяча галактик из
 * слова, связанных рёбрами. Цепочка же ведёт из галактики ровно в одну следующую,
 * то есть выстраивает их в линию, а не в куст, и зерно из неё не совпадает ни с
 * одним узлом. Когда поедут рельсы, ядро будет отдавать СОСЕДА ПО КУСТУ, а эта
 * функция уйдёт вместе с линией.
 */
...
/**
 * Ядро галактики — та самая чёрная дыра. Ворота наружу, и они ОДНИ НА ГАЛАКТИКУ:
 * отдельных «дверей» по системам не бывает, дверь всегда в центре.
 */
