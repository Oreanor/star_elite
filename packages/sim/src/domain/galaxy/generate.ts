 ...
 *
 * Двойная — примерно каждая двадцать пятая: настоящая доля у солнцеподобных
 * выше, около половины, но частая двойная перестаёт быть находкой. Пусть она
 * будет редкостью, на которую наткнёшься, а не фоном. Спутник ТОГО ЖЕ класса —
 * пары рождаются из одного облака и стареют вместе, оттого цвет, светимость и
 * способность отдать топливо у них общие. Размер сопоставимый, но не равный:
 * близнецы до пикселя выглядели бы штампом.
 *
 * Чёрная дыра пары не заводит: у неё и планет-то нет, это дверь, а не звезда.
 */
const BINARY_CHANCE = 0.04
