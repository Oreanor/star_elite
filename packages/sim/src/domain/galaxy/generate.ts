...
/**
 * Спутники. Ноль, один или два — не пять.
 *
 * Каждая луна — это тело в мире: своя зона торможения крейсера, своя кора, о
 * которую разбиваются. Полсотни лун на систему превратили бы её в патоку, где
 * потолок множителя всюду режется ближайшим камнем. Два — это уже система
 * Юпитера в миниатюре, и этого хватает.
 *
 * У газовых гигантов и ледяных миров их больше: там они и в природе водятся.
 *
 * Орбита в РАДИУСАХ планеты. Ближе четырёх — предел Роша и кольца вместо луны,
 * дальше двадцати — луна перестаёт читаться как чья-то и висит сама по себе.
 */
function makeMoons(rng: Rng, planet: string, type: PlanetType): Moon[] {
  const many = type === 'Газовый гигант' || type === 'Ледяная'
  const count = Math.floor(rng() * (many ? 3 : 2))
  return Array.from({ length: count }, (_, i) => ({
    name: moonName(planet, i),
    radius: Math.round(60 + rng() * 180),
    orbit: 4 + rng() * 16 + i * 5,
  }))
}
