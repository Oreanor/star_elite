...
describe('удар о крупное тело', () => {
  ...
  it('на швартовочной скорости станция не таранится', () => {
    const world = quiet()
    const player = world.player
    ram(world, bodyOf(world, 'station'), DOCKING.MAX_SPEED - 5)
    // Общий запас: щит принял бы удар первым, и корпус остался бы цел даже
    // после тарана — по одному корпусу тест не отличил бы одно от другого.
  ...
  /**
   * Быстрый — таранит: запас прочности убывает, корабль отлетает. Проверяется
   * ОТСКОК и сам факт урона, а не его число: перебалансировка
   * `IMPACT.RAM_DAMAGE_PER_SPEED` не должна ронять тест, который к ней не
   * имеет отношения. Считается ОБЩИЙ запас: удар сначала съедает щит.
   */
  it('на скорости станция мнёт корпус и отбрасывает', () => {
    const world = quiet()
    const player = world.player
