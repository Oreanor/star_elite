...
export const AURORA_MK3: Chassis = {
  ...
  hardpoints: [
    // Три ЛАЗЕРНЫХ слота самолётного расклада. Лазер 1 — пара дул из СЕРЕДИНЫ крыльев,
    // лазер 2 — пара с КОНЦОВ, лазер 3 — одно ТОЛСТОЕ дуло из центра (класс до 3: мощь
    // компенсирует одиночность). `offset` — центр установки (для вспышки), бьёт по `nozzles`.
    { offset: [0, -0.25, -1.6], kind: 'gun', maxClass: 2, nozzles: [[-1.9, -0.25, -1.6], [1.9, -0.25, -1.6]] },
    { offset: [0, -0.25, -1.2], kind: 'gun', maxClass: 2, nozzles: [[-3.6, -0.25, -1.2], [3.6, -0.25, -1.2]] },
    { offset: [0, -0.1, -2.2], kind: 'gun', maxClass: 3, nozzles: [[0, -0.1, -2.2]] },
  ...
  slots: [
    ...
    // Два универсальных аукс-слота: сюда идут маскировка, ECM, бомба, скуп ИЛИ
    // миелофон — что выберешь. Пустые с завода (кроме стокового поля в лоадауте).
    { kind: 'aux', maxClass: 3 },
    { kind: 'aux', maxClass: 3 },
  ],
